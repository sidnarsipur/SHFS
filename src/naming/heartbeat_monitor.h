...
constexpr int REPLICATION_FACTOR = 2;
    ...
    void Start() {
        thread_ = std::thread([this] {
            while (!stop_flag_.load()) {
                ...
                if (active_servers.size() < REPLICATION_FACTOR) {
                    spdlog::critical("Not enough active servers for replication");
                    std::this_thread::sleep_for(std::chrono::seconds(interval_));
                    continue;
                }
                ...
                dm->files().write([&](auto &s) {
                    for (auto &[file, servers]: s) {
                        ...
                        if (servers.size() < REPLICATION_FACTOR) {
...
private:
    ...
    static void addReplicationTask(
        ...
        int k = REPLICATION_FACTOR - sources.size();
        if (k > replicas.size()) k = replicas.size();
        std::vector<std::string> selected_replicas(replicas.begin(), replicas.begin() + k);

        // Convert sources to vector for indexing
        const std::vector<std::string> source_list(sources.begin(), sources.end());
